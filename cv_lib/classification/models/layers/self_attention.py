...
from . import EntmaxBisect, get_dropout
...
class MultiHeadSelfAttention(nn.Module):
    def __init__(
        ...
        self.attn_drop = get_dropout(attn_drop)
        self.proj_drop = get_dropout(proj_drop)
        self.attn_identity = nn.Identity()

        self._reset_parameters()

    def get_weight_q(self):
        w = self.qkv.weight[0:self.embed_dim]
        return w

    def get_weight_k(self):
        w = self.qkv.weight[self.embed_dim:2 * self.embed_dim]
        return w

    def get_weight_v(self):
        w = self.qkv.weight[2 * self.embed_dim:3 * self.embed_dim]
        return w
    ...
    def get_bias_q(self):
        b = self.qkv.bias[0:self.embed_dim]
        return b

    def get_bias_k(self):
        b = self.qkv.bias[self.embed_dim:2 * self.embed_dim]
        return b

    def get_bias_v(self):
        b = self.qkv.bias[2 * self.embed_dim:3 * self.embed_dim]
        return b
    ...
    def forward(
        ...
        seq_proj: torch.Tensor = self.qkv(seq)
