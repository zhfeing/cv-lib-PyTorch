...
class APMeter_Base(Meter):
    ...
    def update(
        ...
        gt_hard: List[BoolTensor]
    ):
        """
        Update a batch of predicts. For each img, all tensors must be located on the same device
        """
        device = torch.device("cpu")
        pred_boxes = list_to_device(pred_boxes, device)
        pred_labels = list_to_device(pred_labels, device)
        pred_scores = list_to_device(pred_scores, device)
        gt_boxes = list_to_device(gt_boxes, device)
        gt_labels = list_to_device(gt_labels, device)
        gt_hard = list_to_device(gt_hard, device)
        # to list of dict
        batch_iter = zip(
            pred_boxes,
            pred_labels,
        ...
        batch = list()
        for pred_box, pred_label, pred_score, gt_box, gt_label, gt_hard in batch_iter:
            d = dict(
                pred_box=pred_box,
    ...
    def val_batch(self, batch: List[Dict[str, Tensor]]):
        pass
